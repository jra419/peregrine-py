...
class FiveTMeanSs0 : public Table {
private:
	static constexpr uint32_t NUM_ACTIONS = 21;

	struct key_fields_t {
		// Key fields IDs
		bf_rt_id_t five_t_pkt_cnt;
		bf_rt_id_t priority;
	};
...
public:
	FiveTMeanSs0(const bfrt::BfRtInfo *info,
			  std::shared_ptr<bfrt::BfRtSession> session,
			  const bf_rt_target_t &dev_tgt)
		: Table(info, session, dev_tgt,
				"SwitchIngress_b.stats_five_t_b.mean_ss_0") {
		init_key({
			{"hdr.peregrine.five_t_pkt_cnt", &key_fields.five_t_pkt_cnt},
		...
		for (auto i = 0u; i < NUM_ACTIONS; i++) {
			std::stringstream ss;
			ss << "SwitchIngress_b.stats_five_t_b.rshift_mean_ss_0_";
			ss << i;
...
private:
	...
	void key_setup(uint32_t priority, uint32_t power, uint32_t mask) {
		...
		auto bf_status = key->setValueandMask(key_fields.five_t_pkt_cnt,

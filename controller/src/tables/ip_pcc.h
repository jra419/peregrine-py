...
public:
	IpPcc(const bfrt::BfRtInfo *info,
			  std::shared_ptr<bfrt::BfRtSession> session,
			  const bf_rt_target_t &dev_tgt)
		: Table(info, session, dev_tgt, "SwitchIngress_b.stats_ip_b.pcc") {
		init_key({
			{"ig_md.stats_ip.std_dev_prod", &key_fields.std_dev_prod},
		...
		for (auto i = 0u; i < NUM_ACTIONS; i++) {
			std::stringstream ss;
			ss << "SwitchIngress_b.stats_ip_b.rshift_pcc_";
			ss << i;

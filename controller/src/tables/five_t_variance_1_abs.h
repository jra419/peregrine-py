...
class FiveTVariance1Abs : public Table {
private:
	struct key_fields_t {
		// Key fields IDs
...
public:
	FiveTVariance1Abs(const bfrt::BfRtInfo *info,
				   std::shared_ptr<bfrt::BfRtSession> session,
				   const bf_rt_target_t &dev_tgt)
		: Table(info, session, dev_tgt,
				"SwitchIngress_b.stats_five_t_b.variance_1_abs") {
		init_key({
			{"ig_md.stats_five_t.variance_1", &key_fields.variance_1},
			{"$MATCH_PRIORITY", &key_fields.priority},
		});

		init_actions({
			{"SwitchIngress_b.stats_five_t_b.variance_1_pos",
			 &actions.variance_1_pos},
			{"SwitchIngress_b.stats_five_t_b.variance_1_neg",

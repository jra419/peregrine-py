control c_stats_five_t(inout header_t hdr, inout ingress_metadata_a_t ig_md) {
    ...
    RegisterAction<_, _, bit<32>>(reg_five_t_ss) ract_ss_calc = {
        void apply(inout bit<32> value, out bit<32> result) {
            if (ig_md.stats_five_t.decay_check == 0) {
	...
	RegisterAction<_, _, bit<32>>(reg_five_t_variance) ract_variance_0_write = {
		void apply(inout bit<32> value) {
			value = ig_md.stats_five_t.variance_0;
		}
	};

	RegisterAction<_, _, bit<32>>(reg_five_t_variance) ract_variance_1_read = {
		void apply(inout bit<32> value, out bit<32> result) {
			value = value;
			result = value;
		}
	...
	RegisterAction<_, _, bit<32>>(reg_five_t_last_res) ract_last_res_write = {
		void apply(inout bit<32> value) {
			value = ig_md.stats_five_t.residue;
		}
	};

	RegisterAction<_, _, bit<32>>(reg_five_t_last_res) ract_last_res_read = {
		void apply(inout bit<32> value, out bit<32> result) {
			value = value;
			result = ig_md.stats_five_t.residue - value;
		}
	};
	...
	action ss_calc() {
		ig_md.stats_five_t.ss = ract_ss_calc.execute(ig_md.hash.five_t_0);
	}

	action residue_calc() {
		ig_md.stats_five_t.residue = ig_md.stats_five_t.pkt_len - ig_md.stats_five_t.mean_0;
	}
	...
	apply {
		...
		if (ig_md.stats_five_t.pkt_cnt_0 % 1024 != 0) {

			// Update the current pkt cnt 0 value on pkt cnt register 1.
			ract_pkt_cnt_1_incr.execute(ig_md.hash.five_t_0);

			// Update the stored mean 0 value.
			ract_mean_0_write.execute(ig_md.hash.five_t_0);

			// Update the stored variance 0 value.
			ract_variance_0_write.execute(ig_md.hash.five_t_0);

			// Update the current last residue value.
			ract_last_res_write.execute(ig_md.hash.five_t_0);

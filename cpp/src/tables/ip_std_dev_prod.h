...
class IpStdDevProd : public Table {
private:
...
public:
	IpStdDevProd(const bfrt::BfRtInfo *info,
				 std::shared_ptr<bfrt::BfRtSession> session,
				 const bf_rt_target_t &dev_tgt)
		: Table(info, session, dev_tgt,
				"SwitchIngress_b.stats_ip_b.std_dev_prod") {
		init_key({
			{"ig_md.stats_ip.std_dev_1", &key_fields.std_dev_1},
		...
		for (auto i = 0u; i < NUM_ACTIONS; i++) {
			std::stringstream ss;
			ss << "SwitchIngress_b.stats_ip_b.lshift_std_dev_prod_";
			ss << i;

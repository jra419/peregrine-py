...
class FiveTSumResProdGetCarry : public Table {
private:
	struct key_fields_t {
		// Key fields IDs
...
public:
	FiveTSumResProdGetCarry(const bfrt::BfRtInfo *info,
					  std::shared_ptr<bfrt::BfRtSession> session,
					  const bf_rt_target_t &dev_tgt)
		: Table(info, session, dev_tgt,
				"SwitchIngress_a.stats_five_t_a.sum_res_prod_get_carry") {
		init_key({
			{"ig_md.stats_five_t.decay_check", &key_fields.decay_check},
		});

		init_actions({
			{"SwitchIngress_a.stats_five_t_a.sum_res_prod_get_carry_decay_0",
			 &actions.sum_res_prod_get_carry_decay_0},
			{"SwitchIngress_a.stats_five_t_a.sum_res_prod_get_carry_decay_1",

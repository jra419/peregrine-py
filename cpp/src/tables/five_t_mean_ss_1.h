...
class FiveTMeanSs1 : public Table {
private:
	static constexpr uint32_t NUM_ACTIONS = 21;

	struct key_fields_t {
		// Key fields IDs
		bf_rt_id_t five_t_pkt_cnt_1;
		bf_rt_id_t priority;
	};

	struct actions_t {
		// Actions ids
		std::vector<bf_rt_id_t> rshift_means_ss_1;

		actions_t() : rshift_means_ss_1(NUM_ACTIONS) {}
	};
...
public:
	FiveTMeanSs1(const bfrt::BfRtInfo *info,
			  std::shared_ptr<bfrt::BfRtSession> session,
			  const bf_rt_target_t &dev_tgt)
		: Table(info, session, dev_tgt,
				"SwitchIngress_b.stats_five_t_b.mean_ss_1") {
		init_key({
			{"hdr.peregrine.five_t_pkt_cnt_1", &key_fields.five_t_pkt_cnt_1},
		...
		for (auto i = 0u; i < NUM_ACTIONS; i++) {
			std::stringstream ss;
			ss << "SwitchIngress_b.stats_five_t_b.rshift_mean_ss_1_";
			ss << i;

			auto action_name = ss.str();
			auto *action_id = &actions.rshift_means_ss_1[i];
...
private:
	...
	void key_setup(uint32_t priority, uint32_t power, uint32_t mask) {
		...
		auto bf_status = key->setValueandMask(key_fields.five_t_pkt_cnt_1,

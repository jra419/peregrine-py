...
private:
	...
	struct actions_t {
		// Actions ids
		std::vector<bf_rt_id_t> rshift_covs;

		actions_t() : rshift_covs(NUM_ACTIONS) {}
	};
...
public:
	FiveTCov(const bfrt::BfRtInfo *info,
		  std::shared_ptr<bfrt::BfRtSession> session,
		  const bf_rt_target_t &dev_tgt)
		: Table(info, session, dev_tgt, "SwitchIngress_b.stats_five_t_b.cov") {
		init_key({
		...
		for (auto i = 0u; i < NUM_ACTIONS; i++) {
			std::stringstream ss;
			ss << "SwitchIngress_b.stats_five_t_b.rshift_cov_";
			ss << i;

			auto action_name = ss.str();
			auto *action_id = &actions.rshift_covs[i];
